...
def reg(request):
    if request.method=="POST":
        ...
        if rform.is_valid():
            ...
            Registration(fn=fname,ln=lname,un=un,pswd=pswd,mobile=mobile,email=email).save()
            Registration(fn=fname,ln=lname,un=un,pswd=pswd,mobile=mobile,email=email).save(using='user')
